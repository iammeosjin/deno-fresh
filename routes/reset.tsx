...
export default function Login({ data }: PageProps<Context & { 
	...
	if (props.success) {
		message = (<span class="text-green-700 font-medium p-2 text-sm">{ props.success }</span>)
	}

	if (props.error) {
		message = (<span class="text-amber-700 font-medium p-2 text-sm">{ props.error.message }</span>)
	}
	return (
		<>
			<Head>
							...
							>
								<div class="py-10 px-6 bg-white mx-auto rounded-3xl w-80">
									...
									<div class="space-y-5">
										<div class="relative">
											<input 
												class={ `w-full text-sm px-4 py-3 bg-gray-200 focus:bg-gray-100 rounded-lg border focus:outline-none focus:border-blue-400 ${props.fields.includes('oldPassword') ? "border-red-400" : "border-gray-200"}`} 
												type="password" 
												name="oldPassword"
												value={ props.defaults?.oldPassword ? props.defaults?.oldPassword : '' }
										...
										<div class="relative">
											<input 
												...
												type="password" 
												name="newPassword"
												value={ props.defaults?.newPassword ? props.defaults?.newPassword : '' }
												class={ `w-full text-sm px-4 py-3 bg-gray-200 focus:bg-gray-100 rounded-lg border focus:outline-none focus:border-blue-400 ${props.fields.includes('newPassword') ? "border-red-400" : "border-gray-200"}`} />
										</div>
										<div class="relative">
											<input 
												...
												value={ props.defaults?.confirmPassword ? props.defaults?.confirmPassword : '' }
